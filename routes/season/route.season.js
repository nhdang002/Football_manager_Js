...
router.post('/add', (req, res, next) => {
    ...
    // });;
    const dsStr = req.body.dateStart;
    const deStr = req.body.dateEnd;
    const [dayDs, monthDs, yearDs] = dsStr.split('/').map(part => parseInt(part, 10));
    const [dayDe, monthDe, yearDe] = deStr.split('/').map(part => parseInt(part, 10));

    const DsDate = new Date(yearDs, monthDs - 1, dayDs);
    const DeDate = new Date(yearDe, monthDe - 1, dayDe);
    const timeDifference = DeDate - DsDate;
    const dayDifference = timeDifference / (1000 * 60 * 60 * 24);
    if(timeDifference<=0 || dayDifference<89){
        const messagesSuccess = "ngày kết thúc và ngày bắt đầu không hợp lệ"
        res.render('./home/home', {
            chuyenmuc: 'Tạo hồ sơ giải đấu',
            filename: '../season/add',
            activeAdmin: true,
